...
template<class T>
a_node<T>::a_node( const T& e, handle<T>::compare_fxn& f, 
						 order& o, int index ): handle<T>(f, o, e)
  { ind = index ; }
...
a_node<T>&  a_node<T>::operator=( const a_node<T>& a )
  {
   // SOMETIMES THE NEXT LINE COMPILES OK... SOMETIMES NOT ??!!
//   handle<T>::operator=(a) ; // necessary? -> op= not implemented in handle?
	ind = a.ind ;
	return *this ;
  }
...
void array_heap<T>::swap(handle<T>& h1, handle<T>& h2)
  {
   ...
   // recalculate the ind variable for each node in the heap
	for( int i=0; i<size(); i++ )
		(*array[i]).ind = i ;
 }
...
void array_heap<T>::print(ostream& os, const a_node<T>* n, int k) const
  {
	...
	os << **n << "   ( this=" << (void*)n << ", l=" << array[left((*n).ind)]
	  	<< ", r=" << array[right((*n).ind)] << ", p=" << array[parent((*n).ind)]
      << ")" <<  endl ;
