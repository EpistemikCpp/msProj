...
#ifndef __MHS_HEAP_H__
#define __MHS_HEAP_H__
...
/*******************************************************
  ...
  *
  * must be subclassed - it is the internal building block of a heap */
template<class T>
class handle
{
 public:
...
class heap
{
 ...
 public:
    ...
    virtual handle<T>& push(const T&);
	 ...
	 /* change the position of the element with handle in the heap
	    because its priority has changed asynchronously */
