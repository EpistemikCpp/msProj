...
#include <mmsystem.h>
#pragma message("CGTetris: linking with multimedia library...")
...
// This message is sent to the controlling window, if the music file
// has played off. Note that you will never get this message if you
// have started the music in an infinite loop!
// WPARAM	the pointer to the MIDI object
// LPARAM	not used
#define WM_MIDI_OUTDONE			WM_USER+24
...
// The converter has already encountered a corrupt file and cannot convert
// any more of this file -- must reset the converter
...
struct TRACK
{
	...
	TRACK()	: fdwTrack(0)	, dwTrackLength(0) , pTrackStart(0)
		        , pTrackCurrent(0) , tkNextEventDue(0) , byRunningStatus(0)
	{	}
};
...
// about the target stream buffer and current state of the conversion 
// process to internal lower level conversion routines.
struct CONVERTINFO
{
...
protected:	// implementation
	...
	BOOL	GetTrackByte(TRACK * ptsTrack, LPBYTE lpbyByte)
		{
		 if( DWORD(ptsTrack->pTrackCurrent - ptsTrack->pTrackStart) 
...
protected:	// error handling
		...
		// debuggers output window. Override if you want a different behavior.
...
protected:	// overridables
	...
	// called when a MEVT_F_CALLBACK event is reached 
