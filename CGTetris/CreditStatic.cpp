...
void CCreditStatic::AddBackGround(CDC* pDC, CRect& m_ScrollRect, CRect& m_ClientRect)
{
   ...
   // Draw bitmap in the background if one has been set
	...
	// JK: special version for the tetris game:
	// paint the background image with an offset
	CRect rc;
	GetClientRect(rc);
	ClientToScreen(rc);
	...
	for( int i = 0; i < m_ScrollRect.right; i += m_cxBitmap )
		for( int j = 0; j < m_ScrollRect.bottom; j += m_cyBitmap )
			//JK imageDC.BitBlt( i, j, m_cxBitmap, m_cyBitmap, &tempDC, 0, 0, SRCCOPY );
			imageDC.BitBlt( i, j, m_cxBitmap, m_cyBitmap, &tempDC, xSrc, ySrc, SRCCOPY );
...
BOOL CCreditStatic::SetBkImage(LPCTSTR lpszResourceName)
{
	...
	HBITMAP hBmp = (HBITMAP)::LoadImage( AfxGetInstanceHandle(), 
            lpszResourceName, IMAGE_BITMAP, rc.Width(),rc.Height(), LR_CREATEDIBSECTION );
...
/*
	// after we've created the palette, we resize the image
    CDIBitmap dib;
	dib.CreateFromBitmap(&dc, &m_bitmap);
	CRect rc;
	ClientToScreen(rc);
	CWnd * pParent = GetParent()->GetParent();
	ASSERT(pParent != 0);
