...
//               : CTetrisDlg constructor and method definitions
...
CTetrisDlg::CTetrisDlg(): CBitmapPropPage(CTetrisDlg::IDD, IDS_TitleGamePage)
{
...
BOOL CTetrisDlg::OnInitDialog()
  {
	...
	m_pGameBoard->SetMusicSequence( theApp.GetWantSequence() );
	m_pGameBoard->UseExFigureSet( theApp.GetWantExFigures() );
	m_pGameBoard->SetVolume( theApp.GetVolume() );

	if( theApp.GetMusicType() == IDM_FromFile )
		m_pGameBoard->SetMusicFile( theApp.GetMusicFileName() );
...
{
	// We need to handle WM_SYSKEYDOWN for the F10 key, 
  // but if the ALT key is down too,
	// we do not handle the message ourself ...
	if( pMsg->message == WM_SYSKEYDOWN && (pMsg->lParam & (1<<29)) )
	...
	if( ! (::GetAsyncKeyState(VK_CONTROL) & (1<<(sizeof(SHORT)*8-1))) ) 
		...
		if( m_pGameBoard && 
			...
			switch(pMsg->wParam) 
				{
				case VK_F1:
					m_pGameBoard->SetLevel(1);
					return TRUE;

				case VK_F2:
					m_pGameBoard->SetLevel(2);
					return TRUE;

				case VK_F3:
					m_pGameBoard->SetLevel(3);
					return TRUE;

				case VK_F4:
					m_pGameBoard->SetLevel(4);
					return TRUE;

				case VK_F5:
					m_pGameBoard->SetLevel(5);
					return TRUE;

				case VK_F6:
					m_pGameBoard->SetLevel(6);
					return TRUE;

				case VK_F7:
					m_pGameBoard->SetLevel(7);
					return TRUE;

				case VK_F8:
					m_pGameBoard->SetLevel(8);
					return TRUE;

				case VK_F9:
					m_pGameBoard->SetLevel(9);
					return TRUE;

				case VK_F10:
					m_pGameBoard->SetLevel(10);
					return TRUE;

				case VK_F11:
					m_pGameBoard->SetLevel(11);
					return TRUE;

				case VK_F12:
					m_pGameBoard->SetLevel(12);
					return TRUE;
				}
			}
		...
		if( pMsg->message == WM_KEYDOWN ||
			...
			{ 
			 //TRACE0("sending keydown message to gameboard\n");
				if( m_pGameBoard &&
					  m_pGameBoard->SendMessage(pMsg->message, pMsg->wParam, pMsg->lParam) == 0 )
				return TRUE;
			}
		}
	return CBitmapPropPage::PreTranslateMessage(pMsg);
